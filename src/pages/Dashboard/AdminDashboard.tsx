    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans">
            ...
            <div className="container mx-auto px-6 py-12">
                ...
                <div className="flex gap-4 mb-6 border-b border-slate-200 overflow-x-auto">
                    {(['all', 'pending', 'verified', 'declined'] as const).map(tab => (
                        <button
                            ...
                            className={`pb-3 px-2 text-sm font-bold uppercase tracking-wider transition-all border-b-2 whitespace-nowrap ${activeTab === tab
                ...
                {/* Table */}
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100 overflow-x-auto">
                    <table className="w-full text-left min-w-[1000px]">
                        <thead className="bg-slate-50 border-b border-slate-200">
                            <tr>
                                ...
                                <th className="py-4 px-6 text-xs font-bold text-slate-500 uppercase text-center">AI Score</th>
                            ...
                            {filteredRegistrations.map((reg, index) => (
                                <tr key={reg.id} className="hover:bg-slate-50/50">
                                    ...
                                    <td className="py-4 px-6 text-center">
                                        {reg.resumeScore !== undefined ? (
                                            <div className="flex flex-col items-center">
                                                <span className={`text-lg font-black ${reg.resumeScore >= 15 ? 'text-green-600' : (reg.resumeScore >= 10 ? 'text-yellow-600' : 'text-red-500')}`}>
                                                    {reg.resumeScore}/20
                                                </span>
                                                {reg.resumeAIReason && (
                                                    <span className="text-[9px] text-slate-400 max-w-[150px] truncate block" title={reg.resumeAIReason}>
                                                        {reg.resumeAIReason}
                                                    </span>
                                                )}
                                            </div>
